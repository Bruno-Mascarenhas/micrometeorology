...
| [`docs/allsky-operacional.md`](docs/allsky-operacional.md) | Serving the checkpoints on a CPU workstation: install, which checkpoints to copy, `snapshot` and `watch` per frame and per block with the best/last roles, the on-disk records, a systemd unit, the elevation floor, the first-day domain check |
